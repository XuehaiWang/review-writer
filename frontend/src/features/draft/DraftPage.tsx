...
type DraftPayload = {
  ...
  quality: Record<string, unknown> & { score?: number; goal?: number; paragraph_goal?: number; issues?: QualityIssue[]; hard_gate_failures?: string[]; incremental_evaluations?: IncrementalEvaluation[]; routing?: QualityRouting; manual_claim_review?: ManualClaimReview; unverified_manual_paragraph_ids?: string[]; current?: boolean; status?: string };
        ...
        {tab === "quality" ? <>
          ...
          {issues.map((issue, index) => {
            ...
            return <article id={`quality-issue-${paragraphId}`} tabIndex={-1} className={`quality-issue-react${qualityFocusParagraph === paragraphId ? " quality-focus" : ""}`} key={String(issue.issue_id || index)}>
              ...
              <div className="quality-issue-actions"><button className="button button-secondary" type="button" onClick={() => editIssueParagraph(paragraphId)}>{text("在 Preview 中编辑", "Edit in Preview")}</button>{sectionRepair ? <button className="button button-primary" type="button" disabled={rewrite.isPending || rewriteActive} onClick={() => rewrite.mutate(paragraphId)}>{rewriteActive || (rewrite.isPending && rewrite.variables === paragraphId) ? text("正在自动优化并评分…", "Optimizing and scoring…") : candidate ? text("重新自动优化该段", "Regenerate paragraph repair") : text("自动优化该段", "Optimize this paragraph")}</button> : upstreamRepair ? <button className="button button-primary" type="button" onClick={() => returnToStage(issue.recommended_return_stage, issue)}>{text(`返回${stageLabel(issue.recommended_return_stage)}`, `Return to ${stageLabel(issue.recommended_return_stage)}`)}</button> : <button className="button button-secondary" type="button" disabled={rewrite.isPending || rewriteActive} onClick={() => rewrite.mutate(paragraphId)}>{rewriteButtonText(paragraphId, Boolean(candidate), rewriteActive, true)}</button>}</div>
