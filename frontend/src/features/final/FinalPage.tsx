  ...
  return <main className="workspace page-container workspace-page final-page">
    ...
    {payload ? <><div className="final-grid-react">
      ...
      <section className="pane final-main-react"><div className="pane-head"><div><span className="step-label">{payload.freshness.stale ? text("已过期", "Out of date") : text("当前", "Current")}</span><h2>{tabs.find(([value]) => value === tab)?.[1]}</h2></div></div><div className="final-document-react">
        {payload.final_current && payload.release_current ? <p className={payload.release_ready ? "message message-success" : "message message-warning"}>{payload.release_ready ? text("当前终稿已通过引用身份、书目、分类和图像证据链检查，可作为正式发布候选。", "The current manuscript passed citation identity, bibliography, taxonomy, and figure evidence-chain checks and is release-ready.") : text("当前终稿可继续预览和下载，但仍有未解决的书目、引用或图像证据链问题，暂不标记为可正式发布。请在“发布报告”查看明细并重新生成终稿。", "The current manuscript remains available for preview and download, but unresolved bibliography, citation, or figure evidence-chain issues prevent release-ready status. See the Release report and rebuild after repair.")}</p> : null}
        {tab === "preparation" ? <>
