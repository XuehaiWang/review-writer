...
type ReviewPaper = {
  ...
  human_review?: { reviewed_at?: string; review_note?: string };
};
...
function FigureReview({ projectId, onOpenRedraw, opening, openError }: { projectId: string; onOpenRedraw: () => void; opening: boolean; openError?: Error | null }) {
  ...
  const save = useMutation({
    mutationFn: (candidateIndex: number) => apiRequest<ReviewSelectionResult>(`/api/v1/projects/${encodeURIComponent(projectId)}/figures/review/${encodeURIComponent(selected!.paper_id)}`, { method: "PUT", ...jsonBody({ revision: review.data!.revision, candidate_index: candidateIndex, review_note: note.trim() }) }),
  ...
  return <>
    ...
    {review.data && !review.error ? <>
      ...
      <div className="figure-review-grid-react">
        ...
        <aside className="pane review-note-pane"><div className="pane-head"><div><span className="step-label">{text("选择备注", "Selection note")}</span><h2>{text("审核备注", "Review note")}</h2></div></div><div className="gate-body"><textarea rows={9} value={note} onChange={(event) => setNote(event.target.value)} placeholder={text("选择此图的理由", "Why this figure was selected")} /><p className="muted">{text("备注会与下一次候选选择一起保存。", "The note is saved with the next candidate selection.")}</p>{save.error ? <p className="message message-error">{save.error.message}</p> : null}</div></aside>
      </div>
...
function FigureRedraw({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  const actionError = submit.error || cancel.error || approveOne.error || approveAll.error || confirm.error;
  return <>
