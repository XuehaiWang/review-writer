...
class ProviderSettingsChecks(unittest.TestCase):
    def test_save_masks_secrets_and_applies_environment(self) -> None:
        with tempfile.TemporaryDirectory() as raw, patch.dict(os.environ, RELEVANT_ENV, clear=False):
            ...
            self.assertEqual(result["mineru"]["key_hint"], "********3456")
            self.assertEqual(result["text"]["key_hint"], "********3456")
            self.assertEqual(result["image"]["key_hint"], "********3456")
            self.assertNotIn("api_key", result["mineru"])
            self.assertNotIn("api_key", result["text"])
            self.assertNotIn("api_key", result["image"])
    ...
    def test_settings_page_and_routes_are_wired(self) -> None:
        ...
        self.assertIn("keyState('text',data.text)", html)
        self.assertIn("input.value=saved", html)
        self.assertIn("input.dataset.savedKeyMask=saved", html)
        self.assertIn("function keyPayload(name)", html)
        self.assertIn("saved&&value===saved?'':value", html)
