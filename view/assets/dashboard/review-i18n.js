(function () {
  ...
  const EN_ZH = Object.freeze({
    ...
    "Review merge notes and unresolved human checks together before Stage 9.": "进入第九阶段前，集中检查合并说明和待人工确认事项。",
    ...
    "Merge evidence and open human-review items before Stage 9.": "进入第九阶段前检查合并依据和待人工确认事项。",
    ...
    "none recorded": "未记录问题",
    "Check scientific interpretation, scope boundaries, citations, and every retained figure before continuing to Stage 9.": "进入第九阶段前，请核对科学解释、范围边界、引用以及每一张保留图像。",
    "Merge Report": "合并报告",
    "Issues": "问题",
    "Project Status": "项目状态",
  ...
  const PATTERNS = [
    ...
    [/^Project (.+) is ready: (\d+) unique local papers \((\d+) keyword hits\) across (\d+) keyword groups\.$/, (m) => `项目 ${m[1]} 已就绪：${m[2]} 篇唯一命中文献、${m[3]} 次关键词命中，共 ${m[4]} 个关键词组。`],
    [/^Project (.+) is ready: (\d+) unique local papers \((\d+) keyword hits\) and (\d+) unique external results across (\d+) keyword groups\.$/, (m) => `项目 ${m[1]} 已就绪：${m[2]} 篇唯一命中文献、${m[3]} 次关键词命中、${m[4]} 条唯一外部结果，共 ${m[5]} 个关键词组。`],
    ...
    [/^Transferred from Stage 4 at (.+)$/, (m) => `第四阶段交接时间：${m[1]}`],
