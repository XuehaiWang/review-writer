...
> 文档状态：阶段 A–E 已实现；阶段 F 的代码与计量已实现，待配置 MinerU 服务器令牌后完成真实解析验证；阶段 G–H 未实施
> 编写日期：2026-08-18
> 适用分支：`dy-launch`
...
- 已完成 Token、图像生成次数、MinerU 页数与成本“只记录、不扣费”的用户汇总接口和前端展示。
...
- 管理员在线配置、余额冻结/结算、支付和正式扣费仍属于阶段 G，当前没有伪装成已完成。

本次部署状态：

- PostgreSQL schema 已升级到 `20260818_0010`。
- FastAPI 监听 `0.0.0.0:8770`，Vite 页面保持 `http://192.168.0.5:5175/`。
...
- 后端完整测试 356 个通过、16 个按环境跳过；前端 68 个测试通过，生产构建通过。
- 当前服务器未配置 MinerU Token，因此 MinerU 真实 PDF 解析与供应商页数对账尚未执行；文本与图像 Key 配置状态正常，但本次验证未主动发起会产生供应商费用的真实模型请求。

第一期管理员通过服务器环境变量和受控重启更新供应商配置，不新增管理员后台，避免在核心闭环验证前引入额外配置表和管理页面。
