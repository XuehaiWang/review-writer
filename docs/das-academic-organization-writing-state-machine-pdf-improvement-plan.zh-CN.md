...
- `modern-survey/2` 采用期刊型综述结构：A4 首页以紧凑的题名/作者/机构/摘要/关键词信息框开篇，信息框后立即进入正文，不生成独立封面或目录；
- 正文默认双栏，使用克制的蓝色编号标题、紧凑的正文密度和双栏参考文献；
- Scheme、机制总览图和核心比较表默认跨双栏，普通正文保持双栏阅读节奏，超长表可切换到独立整页 `longtable`；
- 用户提供的 `001_foundation_models_from_task_specific_ai_to_general_intelligence.pdf` 作为结构节奏参考，但不复制其 APRIL Logo、作者身份、具体内容或品牌视觉；
- 显式 Abstract/Keywords 自动进入首页信息框；缺失时只产生自动 QA warning，不新增人工确认，也不编造作者、机构或摘要。
...
`modern-survey` 必须维护英文和中文各一套最小 golden 文稿，覆盖首页题名/摘要/关键词信息框、双栏正文、多级标题、中文/英文混排、公式、跨栏 Scheme、跨栏与跨页长表、脚注和双栏 bibliography。模板或 TeX 发行版变化时执行页面尺寸、元素位置、字体、有效 DPI 和感知图像差异回归；允许的差异阈值随模板版本保存。建议的确定性规则包括：内容 bounding box 不得越出页框；标题位于页面末尾 15% 且后续不足两行正文时产生分页问题；正文/图中文字低于模板最小有效字号产生 warning；照片低于 300 DPI、线图低于 600 DPI 产生资产问题。
