...
def load_paper_to_callout(draft_bundle_path: Path) -> dict[str, str]:
    """Load draft_bundle.json's citation_map ({paper_id: bracket_number}) as a
    paper-ID-to-callout mapping (callout values as strings)."""
    ...
    for paper_id, callout in citation_map.items():
        try:
            mapping[str(paper_id).strip()] = str(int(callout))
...
def identify_trends(matrix: dict[str, Any]) -> dict[str, Any]:
    ...
    Reads the domain-agnostic structured-tag fields (process_type, method,
    input) rather than any hardcoded subject-matter vocabulary, since this
    skill runs across arbitrary review topics.
    """
    papers = matrix.get("papers") if isinstance(matrix, dict) else []
    if not isinstance(papers, list):
        papers = []

    process_types: dict[str, int] = {}
    ...
    for paper in papers:
        if not isinstance(paper, dict):
            continue
        # Process type
        pt = paper.get("process_type") or ""
        if pt and str(pt).strip():
            process_types[str(pt).strip()] = process_types.get(str(pt).strip(), 0) + 1
        # Method
        method = paper.get("method") or ""
        if method and str(method).strip():
            methods[str(method).strip()] = methods.get(str(method).strip(), 0) + 1
        # Year
        y = paper.get("year")
        if isinstance(y, (int, float)):
            ...
            try:
                ...
                pass
        # Input
        inp = paper.get("input") or ""
        if inp and str(inp).strip():
            inputs[str(inp).strip()] = inputs.get(str(inp).strip(), 0) + 1

    top_process_types = sorted(process_types.items(), key=lambda x: -x[1])[:5]
    top_methods = sorted(methods.items(), key=lambda x: -x[1])[:5]
    top_inputs = sorted(inputs.items(), key=lambda x: -x[1])[:5]

    return {
        "dominant_process_types": [{"name": k, "count": v} for k, v in top_process_types],
        "dominant_methods": [{"name": k, "count": v} for k, v in top_methods],
        "dominant_inputs": [{"name": k, "count": v} for k, v in top_inputs],
...
def build_conclusion_prompt(context: dict[str, Any]) -> str:
    ...
    top_process_types = ", ".join(r["name"] for r in trends.get("dominant_process_types", [])[:3])
    top_methods = ", ".join(c["name"] for c in trends.get("dominant_methods", [])[:3])
    top_inputs = ", ".join(s["name"] for s in trends.get("dominant_inputs", [])[:3])
    ...
    prompt = f"""You are an expert scientific review writer. Generate a high-quality Conclusion / Challenges / Insights section for a review paper on: {topic}.

## Instructions

Write 2-3 paragraphs covering:
1. **Overall Conclusions**: Synthesize what the field has collectively achieved across the reviewed papers. Connect dominant approaches to key outputs. Avoid simple enumeration.
2. **Current Challenges / Limitations**: Present 2-3 specific, named challenges drawn from the aggregated limitations. Each challenge must be tied to a specific paper or input/subject class. Avoid vague "more research is needed" language.
...
### Methodology Trends
- Dominant process types: {top_process_types or 'N/A'}
- Dominant methods: {top_methods or 'N/A'}
- Dominant inputs (substrates/starting materials/subjects): {top_inputs or 'N/A'}
...
def run(args: argparse.Namespace) -> int:
    ...
    blueprint_path = project / "02_section_blueprint" / "section_blueprint.json"
    ...
    final_draft_path = project / "07_final_audit" / "final_draft.md"
    section_drafts_path = project / "03_section_drafting" / "section_drafts.md"
    ...
    paper_to_callout = load_paper_to_callout(draft_bundle_path) if draft_bundle_path.exists() else {}
    ...
    if not api_key:
        out_dir = project / "06_conclusion_generation"
