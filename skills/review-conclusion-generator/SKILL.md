...
- The orchestrated input is the approved `05_first_draft/first_draft.md`.
- `05_first_draft/draft_bundle.json`'s `citation_map` field (`{paper_id: bracket_number}`) is the authoritative allowed-paper and numeric-callout map.
...
- Write `conclusion_generated.md` and `conclusion_quality_report.json` under
  `06_conclusion_generation/`; the stage is complete only when both exist and
...
input when available. The approved draft and `draft_bundle.json`'s
`citation_map` remain binding: paper IDs are used internally for grounding,
while rendered prose uses only their mapped numeric callouts.
